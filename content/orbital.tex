...
\begin{lemma}
  ...
  \[ K'_{S,m} \coloneqq \GL_3(\OO_E) \cdot \begin{bmatrix} 0 & 0 & \varpi^m \\ 0 & 1 & 0 \\ \varpi^{-m} & 0 & 0 \end{bmatrix} \]
  denote the orbit of
  $\begin{bmatrix} 0 & 0 & \varpi^m \\ 0 & 1 & 0 \\ \varpi^{-m} & 0 & 0 \end{bmatrix}$
  under the left action of $\GL_3(\OO_E)$.
  Then we have a decomposition
  \[ S_3(F) = \coprod_{m \geq 0} K'_{S,m}. \]
\end{lemma}
The $m=0$ case will be given a special shorthand,
...
One can equivalently define $K'_{S,m}$ to be the part of $S_3(F)$
for which the most negative valuation among the nine entries is $-m$.
...
\begin{itemize}
  ...
  Since $\ell < 2\delta$, we get two cases based on which of
  $\ell + r$ and $k + 2r$ is smaller.
  In the case where $\ell+r > k+2r$, then since $\ell < 2\delta$,
  ...
  \[
    \begin{cases}
      ...
      q^{r+\left\lfloor \frac{k+r-(k+r)\%2}{2} \right\rfloor} + \dots + q^{r+(r+1-k)\%2}
...
\begin{itemize}
  ...
  Insetad our goal is to detect the values of $j$ for which the corresponding value of
  \[ n = 2j-2\delta-2r+k \]
  ...
  \[
    \begin{cases}
      ...
      q^{2\delta+2r-k+\ell} + \dots + q^0
        & \text{if } 2\delta+2r < k \le 2\delta+2r+\ell \\
      0 & \text{otherwise}.
    \end{cases}
  \]
\end{itemize}

%We split the sum based on $0 \le j \le r$ and $r+1 \le j \le \delta+2r$
%to make it more manageable:
%\begin{align*}
%  I_{n>0}^{\text{odd}} =
%  q^{2(\delta+r)s} \sum_{j = 0}^{r} \Bigg(
%    & \sum_{n=1}^{2j-1}
%      q^{n} \left( 1 + q\inv \right) \cdot (-1)^n q^{s(n-2j)} \\
%    &+ \sum_{n=\max(1,2j-r)}^{j}
%      q^{n} \left( 1 + q\inv \right) \cdot (-1)^n q^{s(n-2j)} \\
%    &+ \sum_{n=j+1}^{\ell+j} q^{j} \cdot (-1)^n q^{s(n-2j)} \Bigg) \\
%  {} + q^{2(\delta+r)s}
%  \sum_{j = r + 1}^{\delta + 2r} \Bigg( & \sum_{n=1}^{r}
%      q^{n} \left( 1 + q\inv \right) \cdot (-1)^n q^{s(n-2j)} \\
%    &+ \sum_{n=r+1}^{\min(2j-1-r, \ell+r)}
%      q^{\left\lfloor \frac{n+r}{2} \right\rfloor} \cdot (-1)^n q^{s(n-2j)} \\
%    &+ \sum_{n=2j-r}^{\ell+r} q^{j} \cdot (-1)^n q^{s(n-2j)} \Bigg).
%\end{align*}
%We also have
%\[ I_{n \le 0} = \sum_{i=-r}^{\delta+r} q^{2is}

...
\begin{lemma}
  ...
  \[ u \mapsto h u \lambda^{-1} \]
  for some quaternion $h = \alpha + \beta \Pi \in \DD^\times$
  and an element $\lambda \in E^\times$
  such that $\Nm h = \Norm_{E/F}(\lambda)$.
  ...
  denotes those pairs $(h,\lambda)$ with $\Nm(h) = \Norm_{E/F}(\lambda)$,
...
\begin{align*}
  ...
  g(\Pi) &= (\alpha + \beta \Pi) \cdot \Pi \cdot \lambda\inv
    = \beta \lambda\inv \varpi + \alpha \Pi \lambda\inv
    = \beta \lambda\inv \varpi + \Pi \cdot \bar\alpha \lambda\inv.
\end{align*}
Hence, written as a matrix we have
\[ g = \begin{bmatrix}
  \alpha \lambda\inv & \beta \lambda\inv \varpi  \\
...
\begin{itemize}
  \ii $\Tr g = (\alpha + \bar\alpha)\lambda^{-1}$.
  ...
  \begin{align*}
    ...
    &= \left\langle (\alpha + \beta \Pi)(s + t \Pi)\lambda^{-1}, s + t \Pi \right\rangle \\
    &= \left\langle (\alpha s + \beta \bar t \varpi) \lambda^{-1}
      + (\alpha t + \beta \bar s) \Pi \lambda^{-1},
      s + t \Pi \right\rangle \\
    &= \left\langle (\alpha s + \beta \bar t \varpi) \lambda^{-1}
      + (\alpha t + \beta \bar s) \bar\lambda^{-1} \Pi ,
      s + t \Pi \right\rangle \\
    &= \half \Tr_{\DD/E} [((\alpha s + \beta \bar t \varpi) \lambda\inv
      + (\alpha t + \beta \bar s) \bar\lambda\inv \Pi) (\bar s - t \Pi)] \\
    &= \half \Tr_{\DD/E} \Big[
      ((\alpha s \bar s + \beta \bar s \bar t \varpi) \lambda^{-1}
      - (\alpha t \bar t + \beta \bar s \bar t) \bar\lambda^{-1} \varpi) \\
      &\qquad + ((-\alpha s t - \beta t \bar t \varpi) \lambda\inv
      + (\alpha s t + \beta s \bar s)\bar\lambda\inv) \Pi
    \Big] \\
    &= \half \Tr_{\DD/E} \left[
      \alpha (s \bar s \lambda\inv - t \bar t \varpi \bar\lambda\inv)
      + \beta(s \bar s \bar\lambda\inv - t \bar t \lambda\inv) \Pi \right] \\
    &= \alpha (s \bar s \lambda\inv - t \bar t \varpi \bar\lambda\inv) \\
  \end{align*}
\end{itemize}
\todo{wtf?}
...
\begin{align*}
  ...
  y &\coloneqq (g(\varpi^r))_- = (\alpha_1\sqrt\eps + \beta \Pi) \in \ODT.
\end{align*}

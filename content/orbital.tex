...
\section{Description of the nonzero regions when $n \le 0$}
\begin{claim}
...
\section{Description of the nonzero regions when $n > 0$}
...
\begin{lemma}
  Let $\xi \in \OO_E^\times$, $\rho \in \ZZ$, and $n \ge \max(\rho, 1)$.
  and let $n \ge \rho \ge 1$ be integers.
  Then
  \begin{align*}
...
\todo{Merge this with the previous one tbh, three cases I think}
...
First, note that \eqref{eq:dos} together with \eqref{eq:odd_b_d_zero}
...
However, by \Cref{prop:parameter_constraints}, we always have $\ell < 2 \delta$,
...
\subsection{The case where $n > 0$, $\ell \ge 0$ is even, $v(b)=v(d)=0$}
Suppose $\ell$ is even.
Using the identity

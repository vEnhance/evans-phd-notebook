...
\[ (a + b \Pi) \cdot t = a t + b \Pi t = a t + b \bar t \Pi \]
...
defined by
\[ \left\langle x,y \right\rangle = \half \Tr_{\DD/E}(\bar y x) \]
i.e.\ the projection of $\bar y x \in \DD = E \oplus E\Pi$ onto the first component.
In particular, note that
\[ \left\langle x,x \right\rangle = \bar{x} x = \Nm x \]
or equivalently
...
\begin{lemma}
  ...
  \[ u \mapsto (\alpha + \beta \Pi) u \lambda\inv \]
  for some quaternion $\alpha + \beta \Pi \in \DD^\times$
  and an element $\lambda \in E^\times$
  such that $\Nm(\alpha + \beta \Pi) = \Norm_{E/F}(\lambda)$.

  Moreover, such a description is unique up to multiplication by elements of $F$.
  In other words,
  \[ \U(\VV_2^-) \simeq (\DD^\times \times E^\times)^\circ / \Delta(F^\times) \]
  where $(\DD^\times \times E^\times)^\circ$
  denotes those pairs $(\alpha + \beta \Pi, \lambda)$
  with $\Nm(h) = \Norm_{E/F}(\lambda)$,
...
We have
\begin{align*}
  g(1) &= (\alpha + \beta \Pi) \cdot 1 \cdot \lambda\inv
    = \alpha \lambda\inv + \beta \Pi \lambda\inv
    = \alpha \lambda\inv + \Pi \cdot \bar\beta \lambda\inv \\
  g(\Pi) &= (\alpha + \beta \Pi) \cdot \Pi \cdot \lambda\inv
    = \beta \varpi \lambda\inv  + \alpha \Pi \lambda\inv
    = \beta \varpi \lambda\inv  + \Pi \cdot \bar\alpha \lambda\inv.
\end{align*}
Hence, written as a matrix we have
\[ g = \begin{bmatrix}
  \alpha \lambda\inv & \beta \varpi \lambda\inv \\
  \bar\beta \lambda\inv & \bar\alpha \lambda\inv.
  \end{bmatrix}. \]
...
\begin{itemize}
  ...
  \begin{align*}
    ...
    &= \left\langle (\alpha + \beta \Pi)(s + t \Pi) \lambda\inv, s + t \Pi \right\rangle \\
    &= \left\langle (\alpha s + \beta \bar t \varpi) \lambda\inv
      + (\alpha t + \beta \bar s) \Pi \lambda\inv,
      s + t \Pi \right\rangle \\
    &= \half \Tr_{\DD/E} [
      (\bar s - t \Pi)
      ((\alpha s + \beta \bar t \varpi)
        + (\alpha t + \beta \bar s) \Pi) \lambda\inv ] \\
    &= \half \Tr_{\DD/E} \Big[
      \left( (\alpha s \bar s - \beta t \bar t \varpi)
      + (\alpha s t + \beta s \bar s) \Pi \right) \lambda\inv
    \Big] \\
    &= (\alpha s \bar s - \beta t \bar t \varpi) \lambda \inv \\
    &= \Nm u \cdot \lambda\inv.
  \end{align*}
\end{itemize}
...
\[ \fr \coloneqq \mathbf{1}_{\varpi^{-r} \Mat_2(\OO_E) \cap \U(\VV_n^+)} \in \HH(\U(\VV_2^+)). \]
We also set
\[  \mathbf{1}_{K, r} \coloneqq \fr - \mathbf{1}_{K, \le (r-1)}
  = \mathbf{1}_{K \begin{bmatrix} 0 & \varpi^r \\ \varpi^{-r} & 0 \end{bmatrix} K}. \]
(When $r = 0$, $\mathbf{1}_{K, 0} = \mathbf{1}_K = \mathbf{1}_{K, \le 0}$.)
Analogous to \Cref{sec:hecke_basis_FJ} we have the following result.
\begin{proposition}
...
\[ \alpha \lambda\inv = (\alpha \lambda\inv)_0 + \varsigma \sqrt{\eps} \]
for some $\varsigma \in F$; that is, $\varsigma\sqrt{\eps} \coloneqq (\alpha \lambda\inv)_-$.
Then we use the notation
\begin{align*}
  x &\coloneqq \bar u \sqrt{\eps} u = (s \bar s + t \bar t \varpi) \sqrt{\eps} \in \ODT \\
  y &\coloneqq (g(\varpi^r))_- = \varpi^r(\varsigma\sqrt{\eps} + \beta \Pi \lambda\inv) \in \ODT.
\end{align*}
...
\begin{align*}
  ...
  \left\langle y,y \right\rangle _0 &= \Nm ((g(\varpi^r))_-) \\
  &= \varpi^{2r} (-\varsigma^2\eps - \beta\bar\beta \varpi \lambda^{-2}) \\
  \left\langle x,y \right\rangle _0 &= (\bar y x)_0 = (-yx)_0 \\
  &= \left[
    \varpi^{r} (-\varsigma\sqrt{\eps} - \beta \Pi \lambda\inv)
    \cdot
    \left( (s \bar s + t \bar t \varpi) \sqrt{\eps} \right)
  \right]_0 \\
  &= - \varpi^r \left[
    (s \bar s + t \bar t \varpi) \varsigma \eps
    + \beta (s \bar s + t \bar t \varpi) \sqrt{\eps} \Pi \lambda\inv \right]_0 \\
...
\begin{align*}
  ...
  &= - \eps (\Nm u)^2\cdot \varpi^{2r} (-\varsigma^2 \eps - \beta \bar\beta \varpi \lambda^{-2})
  - (\varpi^r \varsigma \eps (s \bar s + t \bar t \varpi))^2 \\
  &= \eps (\Nm u)^2\cdot \varpi^{2r} \beta \bar\beta \varpi \lambda^{-2} \\
  &= - \eps (\Nm u)^2\cdot \varpi^{2r} (\det g - \alpha \bar\alpha \lambda^{-2}).
\end{align*}

...
\begin{proposition}
  ...
  \[ q^{2 \delta s} \sum_{j=0}^{\delta+2r} q^{-2js}
...
Similarly, the second sum anyway if $j > \left\lfloor \frac{\ell}{2} \right\rfloor + r$.
Therefore we can unify the two parts for the same range $0 \le j \le \delta+2r$ to get
\[
  q^{2\delta s} \sum_{j=0}^{\delta+2r} \left(
    \sum_{n=1}^{\min(2j-1-r, \ell+r, 2\delta+r)}
    q^{\left\lfloor \frac{n+r}{2} \right\rfloor} \cdot (-1)^n q^{s(n+2r-2j)}
    + \sum_{n=\max(1,2j-r)}^{\ell+\min(j,r)}
    q^j \cdot (-1)^n q^{s(n+2r-2j)}
  \right).
\]

\section{Support for the weighted orbital integral for $S_3(F)$}
\label{ch:orbital1}
...
We always retain the setting of \Cref{lem:S3_abcd} throughout this chapter.
...
\begin{proposition}
  [Support of $\mathbf{1}_{\le r}$ for $n=0$]
...
In summary:
\begin{proposition}
  [Support of $\mathbf{1}_{\le r}$ when $n > 0$]
...
\subsection{Rewriting the quadratic constraint on the valuation of $t$ in the $n > 0$ situation}
...
\begin{itemize}
  ...
  If this happens, then as we saw in \Cref{lem:parameter_constraints}
...
\Cref{lem:parameter_constraints}.
However, starting from now on we will have to be a little more careful
and think about which situation of \Cref{lem:parameter_constraints} we are in.

\subsubsection{Analysis of Case 1 and 2 assuming $n > 0$ and $v(b) = 0$}
...
\begin{proof}
  ...
  However, by \Cref{lem:parameter_constraints}, we always have $\ell \le 2 \delta$,
...
\subsubsection{Analysis of merged Case 1 and 2 assuming $n > 0$ and $v(b) > 0$}
...
\subsubsection{Analysis of Case 3\ts+, 3\ts-, 4\ts+, 4\ts- assuming $n > 0$ and $v(b) \ge 0$ and $v(d) \ge 0$}
...
\subsection{Case analysis when $v(b) = v(d) < 0$}
...
\subsubsection{Analysis of merged Case 1 and 2 assuming $v(b) = v(d) < 0$}
...
\subsubsection{Analysis of Case 3\ts+ and Case 4\ts+ assuming $v(b) = v(d) < 0$}

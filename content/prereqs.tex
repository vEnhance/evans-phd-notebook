...
Here's a table of (mostly) reductive groups. Help.
\begin{center}
  \begin{tabular}[h]{ccc}
    $G$ & Radical & Unipotent radical \\ \hline
    $\GL(n)$ & $k$ (diagonal matrices) & $1$ \\
    $\SL(n)$ & $1$ & $1$ \\
    $\opname{O}(n)$ & & $1$ \\
    $\opname{SO}(n)$ & & $1$ \\
    $\opname{Sp}(n)$ & & $1$ \\
    $\opname{U}(n)$ & & $1$ \\
    $\mathbb{G}_\mathrm{a}$ & $\mathbb{G}_\mathrm{a}$ & $\mathbb{G}_\mathrm{a}$
  \end{tabular}
\end{center}

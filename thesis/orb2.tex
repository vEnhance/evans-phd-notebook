...
\begin{align*}
  \sum_{j=\max(0, 1-\lambda+|v(d)|)}^{-2|v(d)|+r-1}
    \sum_{n=j+1}^{\min(j+\lambda, -|v(d)| + r) - |v(d)|}
    q^j \cdot (-1)^n q^{s(n-4|v(d)|+2r-2j)} \\
  + \sum_{j=0}^{-|v(d)|+r-1}
    \sum_{n=\max(j+1, -2|v(d)|+r+1)}^{\min(j+\lambda,r)-|v(d)|}
    q^j (-1)^n q^{s(n-4|v(d)|+2r-2j)} \\

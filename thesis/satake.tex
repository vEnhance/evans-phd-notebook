...
\[ \BC_{S_n}^{\eta^{n-1}} \colon \HH(S_n(F), K') \to \HH(G, K') \]
...
\section{The Satake transformation for $\HH(G',K')$ and $\HH(G,K)$}
...
\begin{theorem}
  ...
  \begin{align*}
    \BC_{S_n} \colon \HH(S_n(F), K') &\xrightarrow{\sim} \HH(G', K') \\
    \BC^{\eta^{n-1}}_{S_n} \colon \HH(S_n(F), K') &\xrightarrow{\sim} \HH(G', K')
  \end{align*}
  such that
  \begin{align*}
...
\begin{tikzcd}
  ...
  \HH(G,K) \ar[r, "\sim"', "\Sat"]
    & \QQ[Y_1^\pm, \dots, Y_m^\pm]^{W_m} \\
    \HH(S_n(F), K') \ar[u, "\sim", "\BC_{S_n}^{\eta^{n-1}}"']
\end{tikzcd}
\end{center}
...
lies in the desired pre-image of the map $\BC \colon \HH(G', K') \to \HH(G,K)$.
...
\begin{tikzcd}
  ...
  \begin{tabular}{c}
    ...
    $\in \HH(S_n(F), K')$
  \end{tabular} \ar[u, "\sim", "\BC_{S_n}"', mapsto]
\end{tikzcd}
\end{center}
...
\begin{proposition}
  ...
  For $n = 3$, we have
  \begin{align*}
    \BC_{S_n} \left( \sum_{j=0}^r \left[ 1 + 2q + 2q^2 + \dots + 2q^{r-j} \right]
    \mathbf{1}_{K'_{S,j}} \right)
    &= \mathbf{1}_{\varpi^{-r} \Mat_3(\OO_E) \cap G} \\
    \BC_{S_n} \left( \mathbf{1}_{K'_{S,r}}

...
%\begin{align*}
%  r_\ast^\eta \colon \HH(G', K') &\to \HH(S_n(F), K') \\
...
\begin{center}
\begin{tikzcd}
  & \HH(G',K') \ar[ld, "r_\ast"'] \ar[r, "\sim"', "\Sat"] \ar[d, "\BC"]
...
\begin{center}
\begin{tikzcd}
  & \begin{tabular}{c} $f'_m + (q-q^2) f'_{m-1}$ \\ $- q^3 f'_{m-2} \in \HH(G',K')$ \end{tabular}
    \ar[ld, "r_\ast"', mapsto] \ar[r, "\Sat", mapsto] \ar[d, "\BC", mapsto]
    & \dots \in \QQ[X_1^\pm, \dots, X_n^\pm]^{\Sym(n)} \ar[d, "\BC", mapsto] \\
  \begin{tabular}{c}
    $\sum_{j=0}^m \big[ 1 + 2q + 2q^2$ \\
    $+ \dots + 2q^{m-j} \big] \mathbf{1}_{K'_{S,j}}$ \\
    $\in \HH(S_n(F), K')$
  \end{tabular} \ar[r, "\sim", "\BC_S"', mapsto]
    & \begin{tabular}{c} $\mathbf{1}_{\varpi{^-m} \Mat_3(\OO_E) \cap G}$ \\ $\in \HH(G,K)$ \end{tabular}
    \ar[r, "\Sat", mapsto]
    & \begin{tabular}{r}
      $q^{2m} \left( Y_1^{\pm} + \dotsb \right)$ \\
      $+ q^{2m-1} \left( Y_1^{\pm(m-1)} + \dotsb \right)$ \\

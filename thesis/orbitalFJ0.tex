...
To define it, fix a self-dual lattice $\Lambda_n$ in $V_0$ of full rank.
First, if $(g,u) \in \U(V_0) \times V_0$ and $f \in \HH(G,K)$,
then we define an orbital integral for $\U(V_0) \times V_0$ by
\begin{equation}
  \Orb^{\U(V_0) \times V_0}\left( (g,u), f \otimes \mathbf{1}_{\Lambda_n} \right)
  \coloneqq \int_{\U(V_0)} f(x\inv g x) \mathbf{1}_{\Lambda_n}(x^{-1} u) \odif x.
  \label{eq:unweighted_orbital_semi_lie}
\end{equation}
That is, in the spirit of \cite[Conjecture 1.9]{ref:liuFJ}
...
\begin{conjecture}
  ...
  If $\guv$ instead matches an element $g \in (\U(V_0) \times V_0)\rs$, then
  \[ \omega\guv \Orb(\phi \otimes \oneV, \guv, 0)
    = \Orb^{\U(V_0) \times V_0}((g,u), \BC^{\eta^{n-1}}_{S_n}(\phi) \otimes \mathbf{1}_{\Lambda_n}) \]
...
but since it is not relevant to this paper we do not pursue this proof here.
...
This case is simpler than the analogous \Cref{sec:param_orbital0}
and we will state our answer in terms of just $v(b)$, $v(c)$, $v(e)$ and $v(d-a)$.
We don't need a further setup like we did in \Cref{sec:param_orbital0}.

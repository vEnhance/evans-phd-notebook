...
\begin{lemma}
  [The matrix of $\partial\Orb$'s has full rank]
...
\begin{proof}
  [Proof of \Cref{lem:semi_lie_ker_full_rank}]
...
\begin{proof}[Proof of \Cref{thm:semi_lie_ker_trivial}]
...
\begin{lemma}
  [A combination vanishing for large $r$]
...
\begin{proof}[Proof of \Cref{thm:semi_lie_ker_huge}]
The first part of \Cref{thm:semi_lie_ker_huge} follows directly from
...
\end{align*}
So we are done by \Cref{lem:chebyshev}.
\end{proof}
...
\begin{proof}[Proof of \Cref{thm:no_kernel_group}]
  Suppose that
...
\begin{itemize}
  ...
  \begin{itemize}
    \ii Apply \Cref{cor:derivative_nn} with $C = 0$, $W = \lambda + 2 \delta$ and $H = \frac{\ell}{2}$.

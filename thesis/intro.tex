...
\begin{conjecture}
  [Kernel conjecture for the orbital on $(S_n(F) \times V'_n(F))\rs^-$]
  ...
  when $\HH(S_n(F))$ is viewed as a ring under the isomorphism of \Cref{ch:satake}.
  \end{enumerate}
\end{conjecture}
...
\begin{theorem}[Explicit formula for the orbital integral on $S_2(F) \times V'_2(F)$]
...
\begin{corollary}[Derivative at $s=0$ for the orbital integral on $S_2(F) \times V'_2(F)$]
...
comes up naturally in \Cref{ch:finale} with a special rose
as the base change mentioned in \cite[Lemma 7.1.1]{ref:AFLspherical}.

\begin{corollary}[The special case $\partial \Orb(\guv, \mathbf{1}_{K'_{S, \le r}} + \mathbf{1}_{K'_{S, \le (r-1)}})$]
...
\begin{theorem}[The kernel of $\partial\Orb$ is essentially injective even for fixed $\gamma \in S_2(F)$]
  \label{thm:semi_lie_ker_trivial}
  Consider any $\gamma \in S_2(F)$ as in \Cref{lem:semi_lie_params}.
...
\begin{theorem}[The kernel of $\partial\Orb$ is large for fixed $(\uu, \vv^\top) \in V'_2(F)$]
...
\begin{remark}
  ...
  of formalizing the notion of ``large kernel'' in \Cref{conj:kernel_semi_lie}.
\end{remark}
...
\begin{theorem}
  [$\partial\Orb \colon \HH(S_3(F)) \to C^\infty(S_3(F)\rs^-)$ has large image]
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    but these formulas are only used for a remark towards the end of \Cref{ch:ker}.
  \end{itemize}
  ...
  We also prove \Cref{thm:no_kernel_group}.
  ...
  \ii Finally, in \Cref{ch:finale} we tie everything together and establish

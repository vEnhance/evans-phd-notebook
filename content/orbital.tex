...
(In other words, the Mastercard logo cannot be drawn.)
\begin{fact}
...
\subsection{Rewriting the quadratic constraint on the valuation of $t$}
...
\begin{itemize}
  ...
  \ii If $\ell < n-r$, then \eqref{eq:2atb_dos} could only hold if $v(2at-b) = \ell/2$.
  ...
  Suppose $v(2at-b+\tau) = \ell/2$, so we need
...
\subsection{Merging of $I_{n \le 0}$ with $I_{n > 0}^{12}$}
We continue assuming $v(b) = v(d) = 0$.
It turns out that $I_{n \le 0} + I_{n > 0}^{12}$ can be rewritten more compactly
...
\begin{itemize}
  ...
  \[ \left\lfloor \frac{r + \ell+r-(\ell+r-k)\%2}{2} \right\rfloor
    = r + \left\lfloor \frac{\ell-(\ell-k)\%2}{2} \right\rfloor.
  \]
  Thus, we obtain
  \[
    \begin{cases}
      ...
      q^{r + \left\lfloor \frac{\ell-(\ell-k)\%2}{2} \right\rfloor}
        + \dots + q^{r + (r+1-k)\%2}
...
\begin{itemize}
  ...
  \begin{align*}
    ...
    2j-2\delta-2r+k \le \lambda+j &\iff j \le 2\delta+2r-k+\lambda \\
    2j-2\delta-2r+k \le \ell+r &\iff j \le \delta+\frac{3r+\ell-k}{2}
  \end{align*}
  The values of $k$ for which there is any valid index $j$
  is always exactly $2\delta + r \le k \le 2 \delta + 2r + \lambda$.
  \todo{need to finish editing stuff after this}
  ...
  We compare this right-hand side to $2\delta+2r$ to get two possible situations.
  In the case where $2\lambda - \ell \le r$, we have
  \[
    \begin{cases}
      ...
      q^{2\delta+2r-k+\lambda} + \dots + q^0
        & \text{if } 2\delta+2\lambda-\ell+r \le k \le 2\delta+\lambda+2r \\
      0 & \text{otherwise}.
    \end{cases}
  \]
  whereas in the case where $2\lambda - \ell > r$, we have
  \[
    \begin{cases}
      q^{\delta+\left\lfloor\frac{3r+\ell-k}{2}\right\rfloor} + \dots + q^{2\delta+2r-k+1}

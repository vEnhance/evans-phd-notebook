...
as the value of the orbital integral described in \Cref{cor:semi_lie_combo}; that is,
...
\end{align*}
and prove \Cref{prop:finale_by_one} by exhausting the
cases based on which value of $N$ is smallest.

\subsection{Proof of \Cref{prop:finale_by_one}
  when $v(e) \le \left\lfloor \tfrac{v(b)+v(c)}{2} \right\rfloor + r$
  and $v(e) \le v(d-a) + r$}
\subsubsection{Case where $r > 0$}
In this case, \Cref{cor:semi_lie_combo} implies that
\begin{align*}
...
\subsubsection{Case where $r = 0$}
If $r = 0$ we refer to \Cref{thm:semi_lie_derivative_single}
...
\subsubsection{Case where $r = 0$}
If $r = 0$, we again use the full \Cref{thm:semi_lie_derivative_single}; we check
\begin{align*}
...
In this case, \Cref{cor:semi_lie_combo} gives
\begin{align*}
  \frac{(-1)^{r+v(c)}}{\log q}\partial\Orb(r, v(b), v(c), v(e), v(d-a))
...
Meanwhile on the Gross-Keating side, we now have
\begin{align*}
...
\end{align*}
This matches the orbital formula, so the proof of \Cref{prop:finale_by_one} is done.
...
To finish, note that \Cref{prop:finale_by_one} immediately implies the following.
\begin{corollary}
...
We now show that \Cref{cor:finale_by_two} implies \Cref{thm:semi_lie_n_equals_2}.
Because $r = 0$ is known already, it suffices to verify for $r > 0$.

The point is that the right-hand side of \Cref{cor:finale_by_two} is exactly
\[ \Int^\circ((g,u), \mathbf{1}_{K, \le r}) - \Int^\circ((g,u) \mathbf{1}_{K, \le r-1})
  = \Int^\circ((g,u), \mathbf{1}_{K, \le r} - \mathbf{1}_{K, \le (r-1)}). \]
Now suppose we sum \Cref{cor:finale_by_two} with $u$ replaced by
...
This matches with \Cref{lem:finale_base_change}.

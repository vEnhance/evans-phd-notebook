...
\begin{itemize}
  ...
  \ii Set
  \[ G \coloneqq \U(\VV_n^-) = \{ g \in \GL_n(\OO_E) \mid g^\ast \beta g = \beta\} \]
...
\section{The specific Hecke algebras $\HH(G',K')$ and $\HH(G,K)$
  for $G' = \GL_n(E)$ and $G = \U(\VV_n^-)$, and the module $\HH(S_n(F), K')$}
For our purposes, we define two Hecke algebras:
\begin{align*}
  \HH(G', K') &\coloneqq \HH(\GL_n(E), \GL_n(\OO_E)) \\
  \HH(G, K) &\coloneqq \HH(\U(\VV_n^-), \U(\VV_n^-) \cap \GL_n(\OO_E))
\end{align*}
...
where the action of $f \in \HH(G', K')$ on $\phi \in \HH(S_n(F), K')$ is given by

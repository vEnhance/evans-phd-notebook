...
\begin{equation}
  1 \leq n \leq \ell + r,
  \qquad \left\lceil \frac{n-r}{2} \right\rceil+\delta+r \leq m \leq \min(n,\ell)+\delta+r
  \label{eq:odd_range2}
\end{equation}
where each $(m,n)$ gives a volume contribution of
...
\begin{align*}
  ...
  &= \kappa \sum_{n=1}^{\min(\ell,2\delta)+r}
    ...
    \cdot \Vol(t_1:-v(t_1)=m)
    \\
  &+ \kappa \sum_{n=1}^{\ell+r}
    \sum_{m=\left\lceil \frac{n-r}{2} \right\rceil+\delta+r}^{\min(n,\ell)+\delta+r}
    q^{-n - (m-\delta-r)} \left( 1 - \frac 1q \right) \cdot (-1)^n q^{s(2m-n)} q^{2n-2m}
    \cdot q^{2m}(1-q^{-2})
    \\
  ...
  &= \sum_{n=1}^{\min(\ell,2\delta)+r}
    ...
    \\
  &+ \sum_{n=1}^{\ell+r}
    \sum_{m=n-\left\lfloor \frac{n+r}{2} \right\rfloor+\delta+r}^{\min(n,\ell)+\delta+r}
    q^{n - (m-\delta-r)} \cdot (-1)^n q^{s(2m-n)}
  .
\end{align*}
To simplify the expressions, we replace the summation variable $m$ with
\[ j \coloneqq (n + \delta + r) - m. \]
...
\[ \sum_{n=1}^{\min(\ell,2\delta)+r}
  ...
  + \sum_{n=1}^{\ell+r}
    \sum_{j=n-\min(n,\ell)}^{\left\lfloor \frac{n+r}{2} \right\rfloor}
...
\begin{itemize}
  ...
  Hence we now have $0 \leq j \leq \left\lfloor \frac{\ell}{2} \right\rfloor + r$.

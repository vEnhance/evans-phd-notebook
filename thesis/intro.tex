...
Part (a) of the conjecture really says that if we consider the map
\begin{align*}
  \HH(S_n(F))
  &\to C^\infty( \left\{ \guv \text{ matching in } \U(\VV_n^-) \times \VV_n^- \right\} ) \\
    \phi &\mapsto \left( \gamma \mapsto \left. \pdv{}{s} \right\rvert_{s=0}
    \Orb \left(\guv, \phi \otimes \oneV, s \right) \right)
\end{align*}
then this map is injective, i.e.\ the image is as large as possible.
...
\begin{itemize}
  ...
  matches an element of $(\U(\VV_2^-) \times \VV_2^-)\rs$.
...
\begin{theorem}
  ...
  Consider the choices of $\gamma \in S_2(F)$ for which $\guv$
  matches with an element of $(\U(\VV_n^-) \times \VV_n^-)\rs$.
...
\begin{theorem}
  ...
  holds for every $\gamma \in S_3(F)\rs$ pairing to an element of $\U(\VV_3^-)\rs$.
\end{theorem}
Hence the map considered in \cite[Conjecture 1.0.2]{ref:AFLspherical}
\todo{use new notation here}
has a large \emph{image} for $n = 3$.

...
\begin{lemma}
  ...
  Let $\xi \in \OO_E^\times$ and let $n \ge \lambda \ge 1$ be integers.
...
The first case is
\begin{equation}
  1 \leq n \leq \min(\ell, 2\delta) + r,
  \qquad n-r \leq m \leq \left\lceil \frac{n-r}{2} \right\rceil+\delta+r - 1
  \label{eq:odd_range1}
\end{equation}
...
\begin{equation}
  1 \leq n \leq \ell + r,
  \qquad \max\left(n-r, \left\lceil \frac{n-r}{2} \right\rceil+\delta+r \right)
    \leq m \leq \min(n,\ell)+\delta+r
  \label{eq:odd_range2}
\end{equation}
where each $(m,n)$ gives a volume contribution of
\[ q^{-n - (m-\delta-r)} \left( 1 - \frac 1q \right). \]
...
\[ 2v(\tau) = \ell = 2v(au-\bar d\bar u) > v(1-u \bar u). \]
...
\begin{itemize}
  ...
  \ii We have $\ell = v(\tau^2) < n-r$ but
  \[ v(2at-b \mp \tau) \geq (n-r)-\ell/2 > 0, \qquad v(2at-b \pm \tau) = \ell/2. \]
  This is two parts, corresponding to the choice of $\pm$.
\end{itemize}
We analyze the second case since the first case is the same as before
(as we have are assuming \eqref{eq:odd_b_d_zero} in this section;
...
\]
Since $(au-\bar d \bar u)^2 - \tau^2 = 4a\bar d(1- \bar u)$,
we agree now to fix the choice of the square root of $\tau$ such that
\begin{equation}
  v(au-\bar d \bar u + \tau) = v(1-u\bar u) - v(\tau) \quad\text{and}\quad
  v(au-\bar d \bar u - \tau) = v(\tau).
  \label{eq:tau_choice}
\end{equation}
This lets us invoke \cite[Lemma 4.7]{ref:AFL} to evaluate $v(4-\Norm(b \pm \tau))$.

In the $\frac{b+\tau}{2a}$ case, we get
\begin{itemize}
  \ii If $m < n - \frac{\ell}{2} + \delta$, the nonempty condition becomes
  \begin{align*}
    v(1-u \bar u) + \delta - \ell = v(4-N(b+\tau)) &\geq n - \frac{\ell}{2} - r \\
    v(1-u\bar u) - \ell/2 = v(au-\bar d \bar u + \tau) & \geq m - \delta - r.
  \end{align*}
  \ii If $m \geq n - \frac{\ell}{2} + \delta$, the nonempty condition becomes
  \begin{align*}
    v(1-u \bar u) &\geq m - \delta - r \\
    v(1-u\bar u) - \ell/2 = v(au-\bar d \bar u + \tau) & \geq n - \frac{\ell}{2} - r.
  \end{align*}
\end{itemize}
Meanwhile, in the $\frac{b-\tau}{2a}$ case, we get
\begin{itemize}
  \ii If $m < n - \frac{\ell}{2} + \delta$, the nonempty condition becomes
  \begin{align*}
    \delta = v(4-N(b-\tau)) &\geq n - \frac{\ell}{2} - r \\
...
\paragraph{First case}
The first case is the union of two regions:
\begin{equation}
  \max(1, \ell+r+1) \leq n \leq -\frac{\ell}{2}+\delta+r + v(1-u \bar u),
  \qquad
  n-r \leq m \leq \max\left( n - \frac{\ell}{2}+\delta, v(1-u\bar u)-\frac{\ell}{2}+\delta+r \right)
...
\begin{equation}
  ...
  \qquad
  n-r \leq m \leq \max\left( n - \frac{\ell}{2}+\delta, \delta+\frac{\ell}{2}+r \right)
  \label{eq:even_case1_minus}
\end{equation}
Note the condition that $m \leq n+\delta+r$ is redundant with
$m \leq n - \frac{\ell}{2} + r$ since $\ell,\delta,r \ge 0$ for $v(b)=v(d)=0$.
\todo{may cause pain for $v(b),v(d) < 0$}

\paragraph{Second case}
The second case is the disjoint union of two bounds:
\begin{equation}
  \leq n \leq
  \qquad
  \leq m \leq
...
\begin{proposition}
  ...
  \[ q^{2(\delta+r)s} \sum_{j=0}^{\delta+2r} q^{-2js}
...
\begin{align*}
  ...
  &= \kappa \sum_{n=1}^{\min(\ell,2\delta)+r}
    \sum_{m=n-r}^{\left\lceil \frac{n-r}{2} \right\rceil+\delta+r-1}
    q^{-n - \left\lceil \frac{n-r}{2} \right\rceil} \left( 1 - \frac 1q \right)
    \\ &\qquad \cdot (-1)^n q^{s(2m-n)} q^{2n-2m}
    \cdot q^{2m}(1-q^{-2})
    \\
  &+ \kappa \sum_{n=1}^{\ell+r}
    \sum_{m=\max\left(n-r, \left\lceil \frac{n-r}{2} \right\rceil+\delta+r\right)}^{\min(n,\ell)+\delta+r}
    q^{-n - (m-\delta-r)} \left( 1 - \frac 1q \right)
    \\ &\qquad \cdot (-1)^n q^{s(2m-n)} q^{2n-2m} \cdot q^{2m}(1-q^{-2})
    \\
  &= \sum_{n=1}^{\min(\ell,2\delta)+r}
    \sum_{m=n-r}^{\left\lceil \frac{n-r}{2} \right\rceil+\delta+r-1}
    q^{-n - \left\lceil \frac{n-r}{2} \right\rceil} \cdot (-1)^n q^{s(2m-n)} q^{2n}
    \\
  &+ \sum_{n=1}^{\ell+r}
    \sum_{m=\max\left(n-r, \left\lceil \frac{n-r}{2} \right\rceil+\delta+r\right)}^{\min(n,\ell)+\delta+r}
    q^{-n - (m-\delta-r)} \cdot (-1)^n q^{s(2m-n)} q^{2n}
    \\
  &= \sum_{n=1}^{\min(\ell,2\delta)+r}
    \sum_{m=n-r}^{\left\lceil \frac{n-r}{2} \right\rceil+\delta+r-1}
  ...
  &+ \sum_{n=1}^{\ell+r}
    ...
    \\
  &= \sum_{n=1}^{\min(\ell,2\delta)+r}
    \sum_{m=n-r}^{n-\left\lfloor \frac{n+r}{2} \right\rfloor+\delta+r-1}
    q^{\left\lfloor \frac{n+r}{2} \right\rfloor} \cdot (-1)^n q^{s(2m-n)}
    \\
  &+ \sum_{n=1}^{\ell+r}
    \sum_{m=\max\left(n-r, n-\left\lfloor \frac{n+r}{2} \right\rfloor+\delta+r \right)}^{\min(n,\ell)+\delta+r}
    q^{n - (m-\delta-r)} \cdot (-1)^n q^{s(2m-n)}
  .
\end{align*}
To simplify the expressions, we replace the summation variable $m$ with
\[ j \coloneqq (n + \delta + r) - m \geq 0. \]
In that case, $2m-n = 2(\delta+n+r-j)-n = n + 2\delta + 2r - 2j$.
Then the expression rewrites as
\begin{align*}
  &\phantom= \sum_{n=1}^{\min(\ell,2\delta)+r}
    \sum_{j=\left\lfloor \frac{n+r}{2} \right\rfloor+1}^{\delta+2r}
    q^{\left\lfloor \frac{n+r}{2} \right\rfloor} \cdot (-1)^n q^{s(n+2\delta+2r-2j)} \\
  &+ \sum_{n=1}^{\ell+r}
    \sum_{j=n-\min(n,\ell)}^{\min\left(\delta+2r, \left\lfloor \frac{n+r}{2} \right\rfloor\right)}
  q^j \cdot (-1)^n q^{s(n+2\delta+2r-2j)}
\end{align*}
We interchange the order of summation so that it is first over $j$ and then $n$.
\begin{itemize}
  \ii The permissible values of $j$ in the second sum
  clearly run from $j=\left\lfloor \frac{r+1}{2} \right\rfloor+1$ to $j=\delta+2r$.
  The constraints for $n$ are that $n \ge 1$, $n \le \min(\ell, 2\delta)+r$,
  and $\left\lfloor \frac{n+r}{2} \right\rfloor + 1 \leq j$,
  which solves to $\frac{n+r}{2} \leq j-\half$ or $n \leq 2j-1-r$.

  \ii The second sum was over pairs $(n,j)$ satisfying $1 \le n \le \ell+r$
  and $j \ge 0$, $j \ge n-\ell$, and $j \le \left\lfloor \frac{n+r}{2} \right\rfloor$.
  Hence we now have
  \[ 0 \leq j \leq \min\left(\delta+2r,
    \left\lfloor \frac{\ell}{2} \right\rfloor + r\right). \]
  Solving for $n$ gives $\max(1, 2j-r) \leq n \leq \ell + r$.
\end{itemize}
Hence we get
\begin{align*}
  &\phantom=\sum_{j=\left\lfloor \frac{r+1}{2} \right\rfloor+1}^{\delta+2r}
    \sum_{n=1}^{\min(2j-1-r, \ell+r, 2\delta+r)}
    q^{\left\lfloor \frac{n+r}{2} \right\rfloor} \cdot (-1)^n q^{s(n+2\delta+2r-2j)}
    \\
  &\qquad+ \sum_{j=0}^{\min\left(\delta+2r, \left\lfloor \frac{\ell}{2} \right\rfloor+r\right)}
    \sum_{n=\max(1,2j-r)}^{\ell+\min(j,r)}
    q^j \cdot (-1)^n q^{s(n+2\delta+2r-2j)}.
\end{align*}
At this point, two of the bounds on $j$ are redundant.
In the first sum, if $j < \left\lfloor \frac{r+1}{2} \right\rfloor + 1$,
the sum is empty anyway.
Similarly, the second sum is empty anyway if
$j > \left\lfloor \frac{\ell}{2} \right\rfloor + r$.
Therefore we can unify the two parts for the same range $0 \le j \le \delta+2r$ to get
\[
  q^{2(\delta+r)s} \sum_{j=0}^{\delta+2r} \left(
    \sum_{n=1}^{\min(2j-1-r, \ell+r, 2\delta+r)}
    q^{\left\lfloor \frac{n+r}{2} \right\rfloor} \cdot (-1)^n q^{s(n-2j)}
    + \sum_{n=\max(1,2j-r)}^{\ell+\min(j,r)}
    q^j \cdot (-1)^n q^{s(n-2j)}
  \right).
\]

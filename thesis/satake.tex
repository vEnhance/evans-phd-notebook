...
\subsection{The map $\BC^{\eta^{n-1}}_{S_n}$}
...
\ifthesis
\subsection{Calculation of $\BC_{S_n}$ when $n = 3$}
...
\fi
\ifpaper
\subsection{Example: Calculation of $\BC_{S_n}$ when $n = 3$}
As an example, we make the base change completely explicit
in the special case $n = 3$, where $m = \left\lfloor n/2 \right\rfloor = 1$.
(In this case $\BC_{S_n}^{\eta^{n-1}} = \BC_{S_n}$ as $\eta^2 = 1$.)
The completed result is \Cref{prop:BC_S3}.
This can be considered a digression and this section will not be needed later.
\fi
...
\subsubsection{Base change from $\HH(\U(\VV_3^+))$ to $\HH(S_3(F))$}
...
\begin{proof}
  ...
  \begin{align*}
    &\phantom= \sum_{j=0}^r \left[ 1 + 2q + 2q^2 + \dots + 2q^{r-j} \right] \mathbf{1}_{K'_{S,j}} \\
    &\qquad - \sum_{j=0}^{r-1} \left[ 1 + 2q + 2q^2 + \dots + 2q^{(r-1)-j} \right] \mathbf{1}_{K'_{S,j}} \\
    &= \mathbf{1}_{K'_{S,r}} +
      \sum_{j=0}^{r-1} \left[ 1 + 2q + 2q^2 + \dots + 2q^{r-j} \right] \mathbf{1}_{K'_{S,j}} \\
    &\qquad - \sum_{j=0}^{r-1} \left[ 1 + 2q + 2q^2 + \dots + 2q^{(r-1)-j} \right] \mathbf{1}_{K'_{S,j}} \\
    &= \mathbf{1}_{K'_{S,r}} + \sum_{j=0}^{r-1} \left[ 2q^{r-j}\mathbf{1}_{K'_{S,j}} \right].

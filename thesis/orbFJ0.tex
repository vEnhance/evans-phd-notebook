...
\begin{theorem}
  ...
  \begin{align*}
    N &\coloneqq \min\left( v(e), \left\lfloor \frac{v(b)+v(c)}{2} \right\rfloor + r, v(d-a) + r \right) \\
...
The formula simplifies a lot if we combine the parts
for $\mathbf{1}_{K'_{S, \le r}}$ and $\mathbf{1}_{K'_{S, \le (r-1)}}$;
this turns out to be right notation later on too.
\begin{corollary}

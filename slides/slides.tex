...
\subtitle{Mass Tech Department of Mathematics}
\author{Evan Chen}
...
\begin{frame}
  \begin{exampleblock}{Main theorem}
    For $n = 2$, this semi-Lie AFL conjecture
    \[
      \Int\left( (g,u), f \right) \log q
      = \pm \left. \pdv{}{s} \right\rvert_{s=0}
      \Orb(\guv, \phi \otimes \oneV, s).
    \]
    holds assuming a certain conjecture about the pullback of
    $\TT_{\mathbf{1}_K \otimes f}(\Delta_{\ZD(u)})$ via Serre tensor construction.
  \end{exampleblock}
  \begin{block}{Strategy of proof}
    \begin{itemize}
      \ii Orbital integral calculated explicitly in Hecke basis.
      \ii Base change and transfer factor are easy.
      \ii On the geometric side, work instead with a Lubin-Tate space $\mathcal{M}_2$
      by pulling back with Serre tensor construction.
    \end{itemize}
  \end{block}
  Removing the conditional hypothesis is a WIP.
\end{frame}
...
\begin{frame}
  \frametitle{Geometric side}
  \begin{itemize}
    \ii Let $\MM_2$ be the Lubin-Tate space for $n=2$ ---
      parametrized by tuples $(Y, \rho)$
    ...
    \ii Use Serre tensor to pull back everything from $\RZ_2$ onto $\MM_2$.
    \ii $\U(\VV_2^-)$ can basically be identified with $\SO(3)$, up to certain tori.
  \end{itemize}
  \begin{theorem}
    [Gross-Keating triple product formula]
    For certain divisors $\ZO4$ defined on $\MM_2 \times \MM_2$, if we set
    \bgroup
    \small
...
\begin{frame}
  ...
  \[ g = \lambda^{-1}
    ...
    \in \U(\VV_2^-) \]
  where $v(\lambda) = 0$.
  With $\mathbb{T}_{\mathbf{1}_{K, r}}$ the Hecke operator on $\MM_2$,
...
\begin{frame}
  ...
  \begin{theorem}
    ...
    \begin{enumerate}
    \item[(a)] Still injective even if one only considers $(\gamma, \uu, \varpi^\ZZ \vv)$.
...
\begin{frame}
  \frametitle{The end}
  Source files and full thesis draft:
  \begin{itemize}

...
\[ H' \coloneqq
  ...
  \cong \GL_2(F). \]
\todo{you seem confused on what you're acting by}
We embed $H'$ into $\GL_3(F)$ by the diagonal embedding
\[ h' \mapsto \left[ \begin{smallmatrix} h' & 0 \\ 0 & 1 \end{smallmatrix} \right] \]
which allows $H'$ to act on $\GL_3(F)$ by conjugation as
\[ g \mapsto h' g \bar{h'}^{-1} \]
which then restricts to an action on $S_3(F)$.

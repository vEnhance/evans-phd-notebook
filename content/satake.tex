...
\begin{center}
\begin{tikzcd}
  & \HH(G',K') \ar[ld, "r_\ast^\eta"'] \ar[r, "\Sat"] \ar[d, "\BC"] & \QQ[X_1^\pm, \dots, X_n^\pm]^{\Sym(n)} \ar[d, "\BC"] \\
  \HH(S_n(F_0, K')) \ar[r, "\sim", "?"'] & \HH(G,K) \ar[r, "\Sat"] & \QQ[Y_1^\pm, \dots, Y_n^\pm]^{W_m} \\
\end{tikzcd}
\end{center}
\todo{describe this}
...
\begin{proposition}
  ...
  \begin{align*}
    &\phantom= r_\ast^\eta(\mathbf{1}_{\Mat_3(\OO_E), v\circ\det=m}) \\
    &= (-1)^m \sum_{j=0}^m \left(
...
\begin{proof}
  ...
  \]
  \todo{this is based on worst valuation, idr the action}
  We calculate
  \begin{align*}
...
\begin{proposition}
  ...
  \[ \Sat\left(\mathbf{1}_{\varpi^{-m} \Mat_3(\OO_E) \cap G}\right)
    = \sum_{i=0}^m q^{2\left\lfloor \frac{m+i}{2} \right\rfloor - 2i + m} Y_1^{\pm i}
  \]
...
\begin{proof}
  ...
  \[ i \le m, \; v(z_1) \ge -m, \; v(z_2) \ge -m, \; v(z_3) \ge -m, \;
  ...
  \[
    ...
    \mathbf{1}_{\varpi^{-m} \OO_F}(z_3)
    \odif{z_1,z_2,z_3}
  \]
  which is equal to
  \[ q^{2\left\lfloor \frac{m+i}{2} \right\rfloor+m}. \]
  Meanwhile, $\delta(\nu(\varpi))^{\half} = q^{-2i}$\todo{find a ref for this}.
  In summary,
  \[
    \Sat\left( \mathbf{1}_{\varpi^{-m} \Mat_3(\OO_E) \cap G}\right) \left( \nu(\varpi) \right)
    =
    \begin{cases}
      q^{2\left\lfloor \frac{m+i}{2} \right\rfloor - 2i + m} & i \le m \\
      0 & i > m
    \end{cases}
  \]
  which is thus the coefficient of $Y_1^{\pm i}$.
\end{proof}

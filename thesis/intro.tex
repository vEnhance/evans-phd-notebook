...
\begin{conjecture}
  ...
  holds for every $(\uu, \vv^\top) \in V_n'(F)$
  such that $\guv$ matches with an element of $(\U(\VV_n^-) \times \VV_n^-)\rs$.

  \item[(b)]
  Fix a choice of $(\uu, \vv^\top) \in V_n'(F)$.
...
\begin{theorem}
  ...
  whose codimension is at most \[ \max(5, v(\vv^\top \uu) + 4). \]
  In addition, part (b) of \Cref{conj:kernel_semi_lie} also holds for $n = 2$;
  that is, the kernel (as a $\QQ$-vector subspace of $\HH(S_2(F))$)
  is not contained in any maximal ideal.
\end{theorem}

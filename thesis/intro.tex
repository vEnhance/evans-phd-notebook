  ...
  \begin{tikzcd}
    ...
    \text{GGP \cite{ref:GGP}}
      ...
      & \ar[l, Rightarrow, "\text{used to prove}"] \text{FL \cite{ref:JR}}
        \ar[d, dotted, leftrightarrow, "\text{analog}"]
      & \ar[l] \text{\cite{ref:leslie}}
        \ar[d, dotted, leftrightarrow, "\text{analog}"] \\
    \text{Arith.\ GGP \cite{ref:GGP}}
      & \ar[l, Rightarrow, "\text{used to prove}"] \text{AFL \cite{ref:AFL}}
      & \ar[l] \text{\cite{ref:AFLspherical}} \\
...
The part of the calculation involving orbital integral
uses an explicit calculation that has two parts:
\begin{itemize}
  \ii The calculation also makes $\BC_{S_{n}}^{\eta^{n-1}}$
  ...
  \[ \Orb(\gamma, f') \]
  for any $\gamma \in S_n(F)$ and $f'$, in terms of invariants of $\gamma$
  and a decomposition of $f'$ in a natural basis.

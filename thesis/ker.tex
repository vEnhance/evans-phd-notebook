\section{Large kernel and large image}
\label{ch:ker}
...
\subsection{A sequence of test functions almost lying in the kernel in the semi-Lie case}
...
\end{proof}
\subsection{Proof of \Cref{thm:no_kernel_group} for the group AFL}
...
To prove it, we first state the following corollaries
of \Cref{lem:derivative_nn} and \Cref{lem:derivative_cc}, respectively.
\begin{corollary}
...
\begin{proof}
  [Proof of \Cref{thm:group_kernel_full}]
  ...
  using the same inputs as the proof of \Cref{thm:S3_orbital_deriv}.
...
\subsubsection{Generation of $\HH(S_3(F))$ as an ideal}

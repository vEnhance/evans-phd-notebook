...
\begin{lemma}
  ...
  \[ g = \begin{bmatrix}
    ...
    \lambda^{-1}
  \]
  and $u = s + t \Pi$ as above, and suppose $(g,u)$ matches with
  ...
  \begin{align*}
    \Nm u &= e \\
    \alpha \lambda\inv &= a \\
    \bar\alpha \lambda\inv &= d \\
    \beta \bar\beta \varpi \lambda^{-2} &= bc.
  \end{align*}
\end{lemma}
\begin{proof}
  Setting the invariants above equal we obtain
  \begin{align*}
    a + d &= (\alpha + \bar\alpha) \lambda\inv \\
    \det \gamma = ad - bc &= \det g \\
    e &= \Nm u \\
    de &= \alpha\lambda\inv \Nm u.
  \end{align*}
  Hence it follows $d = \alpha \lambda\inv$ and $a = \bar\alpha \lambda\inv$.
  In particular $ad = \alpha \bar \alpha \lambda^{-2}$
  and hence
  ...
  proving the lemma.
\end{proof}
\todo{Need to verify $v(d- \bar d) = v(d-a)$.}
...
\begin{align*}
  ...
  y &\coloneqq (g(\varpi^r))_- = (\varsigma\sqrt\eps + \beta \Pi \lambda\inv) \in \ODT
\end{align*}

\chapter{Evaluation of the orbital integral}
We now proceed to actually do the calculation of the orbital integral.
...
\section{Contribution from Case 1 and Case 2 assuming $v(b)=v(d)=0$}
...
We describe the final result for odd $\ell$ in the next section.

\section{Final result for $\ell$ odd}
...
\section{Contribution from Case 3\ts{+}, 3\ts{-}, 4\ts{+} assuming $v(b) = v(d) = 0$}
...
\section{Final answer for even $\ell$, when $v(b) = v(d) = 0$}
\begin{theorem}
  ...
  \[
    ...
    \end{cases}
  \]
  and the piecewise function $\cc_\gamma \colon \ZZ \to \ZZ_{\ge 0}$ is defined by by setting the constants
  \begin{align*}
  ...
  \[
    ...
    \begin{cases}
      k - (\ell - r)
        & \text{if } \ell-r \le k \le \delta + \frac{\ell}{2} - r \\

...
Later in this paper we will see it holds for $n = 3$ as well.
\begin{conjecture}
  ...
  \begin{align*}
    \HH(S_n(F))
    &\to C^\infty( \left\{ \text{elements of } S_n(F)\rs \text{ matching } \U(\VV_n^-)\rs \right\} ) \\
  ...
  in the sense that the kernel is not contained in any maximal ideal of $\HH(\U(\VV_n^-))$.
\end{conjecture}
...
In fact, even for $n=2$ it turns out the different parameters
behave quite differently from each other,
in contrast to the group case from before.
Thus, we propose the following conjecture, which we have proved for $n = 2$
...
\begin{conjecture}
  ...
  Then there are no nonzero functions $\phi \in \HH(S_n(F))$ for which
  \[ \left. \pdv{}{s} \right\rvert_{s=0}
    \Orb \left(\guv, \phi \otimes \oneV, s \right) = 0 \]
  holds for every such $(\uu, \vv^\top) \in V'_n(F)$.
...
\begin{theorem}
  ...
  Consider any $\gamma \in S_2(F)$
  such that $\left( \gamma, \begin{pmatrix} 0 \\ 1 \end{pmatrix}, \begin{pmatrix} 0 & 1 \end{pmatrix} \right)$
  matches an element of $\U(\VV_2^-) \times \VV_2^-$.
...
\begin{theorem}
  ...
  Assume that $\gamma$ matches an element in $\U(\VV_3^-)\rs$ (resp.\ $(\U(\VV_2^-) \times \VV_2^-)\rs$).

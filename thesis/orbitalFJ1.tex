...
\begin{itemize}
  ...
  Therefore \eqref{eq:case_6_plus_m_raw} can be rewritten as just
  ...
  \]
  to hold.
  But the latter inequality is always false, so this is just requiring
  \[ m \le n_2 + \frac{\theta}{2} + r. \]
\end{itemize}
If so, the locus of valid $y$ has volume
\[ q^{-\max(m-n_2, m - \frac{\theta}{2} - r, 0) - v(c) - r}. \]
Hence the volume element contributed by this portion is exactly given by
\begin{align*}
  \left\lvert \frac{x_1}{x_2} \right\rvert
  q^{-\max\left( m-n_2, m/2 - \frac{\theta}{2} - r, 0 \right) + v(c) + r}
...
(which overlap, but they give different values of $y$):
\begin{align*}

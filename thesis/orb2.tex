...
\begin{itemize}
  ...
  running form $k = \frac{\ell}{2} - r$ to $k = \frac{\ell}{2} + 2 \delta + r$.
\end{itemize}
...
\begin{itemize}
  ...
  \begin{align*}
    ...
    &\coloneqq \begin{cases}
      ...
      - \frac{\ell}{2} + \delta + r - \left\lceil \frac{k+r-\lambda}{2} \right\rceil + 1 &\text{if } k \ge 2 \delta + r \\
    \end{cases} \\
    &= \begin{cases}
...
\begin{figure}[ht]
  ...
  \begin{asy}
    ...
    dot("$2\delta+r$", (9,3*h), grey dir(90));
  ...
  \caption{Rough illustration of the regions for $C_1$, $C_2$, $C_3$, $C_4$.
...
which effectively lets us not consider cases on $\lambda-r$ or $2\delta+r$.
\begin{lemma}
  ...
  \[
    ...
    \end{cases}
  \]
  \ii For $k \ge \max\left( \lambda - \frac{\ell}{2} + \delta - r, 2 \delta + r \right)$,
  \[
    \left\lfloor \frac{\lambda-\ell+2\delta+r-k+1}{2} \right\rfloor
    = \begin{cases}
      C_2(k) &\text{if } k \ge 2\delta + r \\
      C_2(k)+C_4(k) & \text{if }k \le 2 \delta + r.
    \end{cases}
  \]
  \end{itemize}
\end{lemma}
...
\begin{enumerate}
  \ii Suppose $\ell - r + 1 \le k \le \frac{\ell}{2} + \delta - r$
  (in particular, also $k \le 2 \delta + r$
  and $k \le \lambda - \frac{\ell}{2} = \delta$).
  \begin{itemize}
    ...
    \begin{align*}
      ...
      &= k - (\ell-r) + (k+r) \% 2.
    \end{align*}
    \ii If $k \ge \lambda - r$, then we instead get
    \begin{align*}
      &\phantom= C_2(k) + C_3(k) + C_4(k) \\
      &=      + \left\lfloor \frac{k-\ell+r+1}{2} \right\rfloor \\
      &= \frac{\lambda-\ell+1}{2} - (k+r-\lambda) \% 2
      + \frac{k-\lambda+r+1}{2} + \frac{k-\ell+r+1}{2} - \half \\
      &= k-(\ell-r) + 1 - (k+r-\lambda) \% 2 \\
      &= k - (\ell - r) + (k+r) \% 2
    \end{align*}
    which is the same as in the other case, so we may fold these cases together.
  \end{itemize}

  \ii Next suppose that
  $\frac{\ell}{2} + \delta - r \le k \le \min(\delta + \frac{\ell}{2} + r,
    \lambda + \delta - \frac{\ell}{2} - r - 1)$.
  \begin{itemize}
    \ii If $k < \lambda - r$, the contribution is
    \begin{align*}
      C_1(k) + C_4(k)
      &= \left\lfloor \frac{k - \ell + r + 1}{2} \right\rfloor
      + \frac{-\frac{\ell}{2} + \delta + 1 - (k+\frac{\ell}{2}+\delta+r)\%2 - (k+r+1)\%2}{2}
    \end{align*}
    \ii If $k \ge \lambda-r$ we instead get
    \begin{align*}
      &\phantom= C_2(k) + C_3(k) + C_4(k) \\
      &= \frac{\lambda-\ell+1}{2} - (k+r-\lambda) \% 2
      + \left\lfloor \frac{k-\lambda+r+1}{2} \right\rfloor

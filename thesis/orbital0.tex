...
\label{sec:orbital0_hecke_basis}
...
Again, assume $n = 3$.
Further assume $\gamma \in S_3(F)$ is regular.
We identify some parameters for the orbit of $\gamma$
that we can use for our explicit calculations.

\subsubsection{Rewriting the orbital integral as a double integral over $E$
...
We assume henceforth that $\gamma$ is regular (cf.\ \Cref{def:regular}).
...
cover all the \emph{regular} orbits.
